    ...
    <div className="p-4 md:p-8">
      ...
      <div style={{ border: "0.5px solid #D5D2BF" }}>
        {products.length === 0 ? (
          ...
          products.map((p) => {
            // ◄ DYNAMIC RESOLUTION: Safely extract and format size configurations
            const stockMap = (p.sizesStock || {}) as Record<string, number>;
            const totalItemsLeft = Object.values(stockMap).reduce(
              (sum, val) => sum + (Number(val) || 0),
              0,
            );
            const breakdownText = `M:${stockMap.M || 0} · L:${stockMap.L || 0} · XL:${stockMap.XL || 0} · 2XL:${stockMap["2XL"] || 0}`;
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p
                    className="text-xs tracking-[0.08em] uppercase font-mono opacity-80"
                    style={{ color: "#3D4A28" }}
                  >
                    {p.color} · {breakdownText} ({totalItemsLeft} total)
                  </p>
                </div>
